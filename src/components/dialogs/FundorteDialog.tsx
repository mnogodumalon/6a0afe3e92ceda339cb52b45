...
interface FundorteDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Fundorte['fields']) => Promise<void>;
  defaultValues?: Fundorte['fields'];
  enablePhotoScan?: boolean;
  enablePhotoLocation?: boolean;
}

export function FundorteDialog({ open, onClose, onSubmit, defaultValues, enablePhotoScan = true, enablePhotoLocation = true }: FundorteDialogProps) {
  const [fields, setFields] = useState<Partial<Fundorte['fields']>>({});
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      const clean = cleanFieldsForApi({ ...fields }, 'fundorte');
      await onSubmit(clean as Fundorte['fields']);
      onClose();
    } finally {
      setSaving(false);
    }
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "ortsname": string | null, // Name des Ortes\n  "typ": LookupValue | null, // Typ der Quelle (select one key: "sammelkarten" | "online_shop" | "geschaeft" | "videospiel" | "tausch" | "geschenk" | "event" | "sonstiges") mapping: sammelkarten=Sammelkarten, online_shop=Online-Shop, geschaeft=Geschäft, videospiel=Videospiel, tausch=Tausch, geschenk=Geschenk, event=Event, sonstiges=Sonstiges\n  "datum": string | null, // YYYY-MM-DD\n  "notizen": string | null, // Notizen\n}`;
      ...
      setFields(prev => {
        ...
        return merged as Partial<Fundorte['fields']>;
      });
      setAiText('');
      setScanSuccess(true);
  ...
  const DIALOG_INTENT = defaultValues ? 'Fundorte bearbeiten' : 'Fundorte hinzufügen';
      ...
      <DialogContent className="max-w-lg max-h-[92vh] flex flex-col overflow-hidden p-0 gap-0">
        ...
        <form onSubmit={handleSubmit} className="flex flex-1 flex-col min-h-0">
          ...
          <div className="space-y-1.5">
            <Label htmlFor="ortsname">Name des Ortes</Label>
            <Input
              id="ortsname"
              value={fields.ortsname ?? ''}
              onChange={e => setFields(f => ({ ...f, ortsname: e.target.value }))}
            />
          </div>
          <div className="space-y-1.5">
            <Label htmlFor="typ">Typ der Quelle</Label>
            <Select
            ...
            >
              ...
              <SelectContent>
                ...
                <SelectItem value="videospiel">Videospiel</SelectItem>
          ...
          <div className="space-y-1.5">
            <Label htmlFor="datum">Datum des Erwerbs</Label>
            <Input
              id="datum"
              type="date"
              value={fields.datum ?? ''}
