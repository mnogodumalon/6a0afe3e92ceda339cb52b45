...
interface EntwicklungsstufenDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Entwicklungsstufen['fields']) => Promise<void>;
  defaultValues?: Entwicklungsstufen['fields'];
  enablePhotoScan?: boolean;
  enablePhotoLocation?: boolean;
}

export function EntwicklungsstufenDialog({ open, onClose, onSubmit, defaultValues, enablePhotoScan = true, enablePhotoLocation = true }: EntwicklungsstufenDialogProps) {
  const [fields, setFields] = useState<Partial<Entwicklungsstufen['fields']>>({});
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      const clean = cleanFieldsForApi({ ...fields }, 'entwicklungsstufen');
      await onSubmit(clean as Entwicklungsstufen['fields']);
      onClose();
    } finally {
      setSaving(false);
    }
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      setFields(prev => {
        ...
        return merged as Partial<Entwicklungsstufen['fields']>;
      });
      setAiText('');
      setScanSuccess(true);
  ...
  const DIALOG_INTENT = defaultValues ? 'Entwicklungsstufen bearbeiten' : 'Entwicklungsstufen hinzufügen';
      ...
      <DialogContent className="max-w-lg max-h-[92vh] flex flex-col overflow-hidden p-0 gap-0">
        ...
        <form onSubmit={handleSubmit} className="flex flex-1 flex-col min-h-0">
          ...
          <div className="space-y-1.5">
            <Label htmlFor="name">Name der Entwicklungsstufe</Label>
            <Input
              id="name"
              value={fields.name ?? ''}
          ...
          <div className="space-y-1.5">
            ...
            <Input
              ...
              type="number"
              value={fields.level ?? ''}
              onChange={e => setFields(f => ({ ...f, level: e.target.value ? Number(e.target.value) : undefined }))}
            />
          </div>
          <div className="space-y-1.5">
